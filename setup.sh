...
if [ ! -f "$WORKDIR/docker-compose.yml" ]; then
  print_info "Downloading Fusionaly DevBox to current directory"
...
# Helper to run commands in fusionaly-web container (use direct docker exec after containers are healthy)
run_in_container() {
  docker exec fusionaly-devbox sh -lc "$1" 2>/dev/null || return 1
}

print_info "Starting DevBox services..."
# Clean up any existing containers with fixed names (best-effort)
docker rm -f fusionaly-devbox >/dev/null 2>&1 || true
...
if [ "$ENV_MODE" != "test" ]; then
  print_error "DevBox must run with FUSIONALY_ENV=test (got: '$ENV_MODE'). Aborting to prevent misuse."
  exit 1
fi
...
if run_in_container "sqlite3 /app/storage/fusionaly-test.db \"INSERT INTO settings (key, value, created_at, updated_at) VALUES ('license_key','IM-DEVBOX-TEST-ONLY', datetime('now'), datetime('now')) ON CONFLICT(key) DO UPDATE SET value='IM-DEVBOX-TEST-ONLY', updated_at=datetime('now');\"" >/dev/null 2>&1; then
...
echo ""
echo "🎉 Fusionaly DevBox is Ready!"
echo "==================================="
...
print_status "Logs:     docker logs -f fusionaly-devbox"
